  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 space-y-5">
        <h2 className="text-sm font-semibold text-gray-300 uppercase tracking-wider">Cores da marca</h2>
          ...
          {COLOR_LABELS.map(({ key, label, hint }) => (
            <div key={key} className="space-y-2">
              ...
              <div className="flex items-center gap-3 bg-gray-800 border border-gray-700 rounded-xl p-3">
                <label className="relative w-10 h-10 rounded-lg cursor-pointer overflow-hidden flex-shrink-0 shadow-inner border border-gray-600">
                  <input
                    type="color"
                ...
                <input
                  ...
                  className="flex-1 bg-transparent text-gray-300 text-sm font-mono focus:outline-none w-0"
                  maxLength={7}
                />
              </div>
            </div>
        ...
        <div className="rounded-xl overflow-hidden border border-gray-700">
          <div className="px-4 py-2 bg-gray-800 border-b border-gray-700">
            <p className="text-xs text-gray-500">Preview das cores</p>
          </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 space-y-4">
        <h2 className="text-sm font-semibold text-gray-300 uppercase tracking-wider">Logo da marca</h2>

        {state.logoUrl ? (
          <div className="relative inline-flex">
            <div className="w-32 h-32 rounded-xl bg-gray-800 border border-gray-700 flex items-center justify-center overflow-hidden">
          ...
          <div>
            ...
            <button
              ...
              disabled={uploading}
              className="w-full border-2 border-dashed border-gray-700 hover:border-indigo-500 rounded-xl p-8 flex flex-col items-center gap-3 transition-all group disabled:opacity-50"
            >
              <div className="w-12 h-12 rounded-xl bg-gray-800 group-hover:bg-indigo-600/20 flex items-center justify-center transition-all">
                {uploading ? (
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-gray-600 mt-1">PNG, JPG, SVG até 5 MB</p>
              </div>
            </button>
      ...
      {/* Estilo dos slides */}
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 space-y-4">
        <h2 className="text-sm font-semibold text-gray-300 uppercase tracking-wider">Estilo dos slides</h2>
          ...
          {SLIDE_STYLES.map(style => (
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className={`font-semibold text-sm ${state.slideStyle === style.value ? 'text-pink-300' : 'text-gray-200'}`}>
                  {style.label}
                </span>
                {state.slideStyle === style.value && <span className="ml-auto text-pink-400 text-xs">✓</span>}
              </div>
      ...
      <div className="flex gap-3">
        ...
        <button
          ...
          className="flex-1 bg-gradient-to-r from-pink-600 to-rose-600 hover:from-pink-500 hover:to-rose-500 disabled:opacity-60 text-white font-semibold py-4 rounded-xl transition-all shadow-lg shadow-pink-500/25 flex items-center justify-center gap-2"
        >
          {loading ? (
            <>
